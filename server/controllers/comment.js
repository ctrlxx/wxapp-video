...
module.exports = {
  ...
  favorites: async ctx => {
    const { userinfo } = ctx.state.$wxInfo
    const comments = await DB.query('SELECT d.id, d.type, d.content, b.image, b.title, c.user_info FROM favorites a, movies b, cSessionInfo c, comments d WHERE a.open_id = c.open_id and a.comment_id = d.id and d.movie_id = b.id and c.open_id = ?', [userinfo.openId])
    ctx.state.data = comments
  }
}